...
MODEL_DIR = "/models"
HF_AUTH_TOKEN = os.getenv("HF_AUTH_TOKEN", "your_default_token_here")
os.environ["TRANSFORMERS_CACHE"] = CACHE_PATH
os.environ["HF_HOME"] = CACHE_PATH
os.environ["TORCH_HOME"] = CACHE_PATH
os.environ["HF_AUTH_TOKEN"] = HF_AUTH_TOKEN  # Ensure token is set

transcription_threshold = 90

audio_url = "https://s3.eu-central-2.wasabisys.com/qira/658/2024/7/_ovjeku_se_plae__bacio_sam_deset_ton_1722256407942/_ovjeku_se_plae__bacio_sam_deset_ton_1722256407942.mp3"
...
def pre_download_align_model(language_code="en"):
    ...
    if not model_files_exist:
        try:
            ...
            logger.error(f"Failed to download or load the model: {e}")
    else:
...
def pre_download_diarization_model():
    ...
    try:
        diarization_model = Pipeline.from_pretrained(model_name, use_auth_token=HF_AUTH_TOKEN)
...
whisperX_image = (
    ...
    .run_commands(
        [
            ...
            "python3 -c 'from transformers import Wav2Vec2ForCTC, Wav2Vec2Processor; processor = Wav2Vec2Processor.from_pretrained(\"voidful/wav2vec2-xlsr-multilingual-56\", use_auth_token=os.environ[\"HF_AUTH_TOKEN\"], cache_dir=\"/build_cache\"); model = Wav2Vec2ForCTC.from_pretrained(\"voidful/wav2vec2-xlsr-multilingual-56\", use_auth_token=os.environ[\"HF_AUTH_TOKEN\"], cache_dir=\"/build_cache\")'",
        ]
    )
...
async def transcribe_audio_endpoint(
    ...
    result = transcribe_and_align_audio(file_contents, transcription_request.language,
                                        transcription_request.output_formats)
...
def transcribe_and_align_audio(file_contents: bytes, language: str, output_formats: list, custom_dictionary: str = ""):
    ...
    asr_options = {
        "initial_prompt": custom_dictionary,
    }
    ...
    aligned_result = whisperx.align(all_transcriptions, model_a, metadata, temp_audio_file_path, device,
    ...
    distance_from_whisper = compare_paragraphs(cyrillic_to_latin(transcription_text), whisper_vtt_text)
    ...
    if distance_from_whisper > transcription_threshold:
        ...
        print(f"Distance between transcription and vtt text is smaller than 90, calling Gladia. Distance: {distance_from_whisper}")
        transcript = transcribe_audio(audio_url)
        gladia_transcript_text = transcript['result']['transcription']['full_transcript']
        distance_from_gladia = compare_paragraphs(cyrillic_to_latin(transcription_text), gladia_transcript_text)
...
def write_vtt(segments, path, language):
    subtitles_processor = whisperx.SubtitlesProcessor(segments, language)
...
if __name__ == "__main__":
    ...
    async def test_transcribe_audio():
        transcription_request = TranscriptionRequest(
            file_url="https://s3.eu-central-2.wasabisys.com/qira/658/2024/7/_ovjeku_se_plae__bacio_sam_deset_ton_1722256407942/_ovjeku_se_plae__bacio_sam_deset_ton_1722256407942.mp3",
            language="sr",
